...
#     optimize_svc(data, targets)
    ...
    print(Colours.green("--- Optimizing Logistic Regression ---"))
    
    # skf = RepeatedStratifiedKFold(n_splits=10, n_repeats=10)

    X_train, X_val, y_train, y_val = train_test_split(train_x, train_y, test_size=0.10, random_state=5342792)

    estimator = LogisticRegressionCV(dual=False, n_jobs=-1).fit(X_train, y_train)

    # cval = cross_val_score(estimator, train_x, train_y,
    #                        scoring='accuracy', cv=skf)

    print(f'CV Accuracy Score : {estimator.score(X_val, y_val)}')
